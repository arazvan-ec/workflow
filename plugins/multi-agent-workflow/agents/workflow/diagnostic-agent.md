...
category: workflow
context: fork
description: "Analyzes repeated failures in the Ralph Wiggum Loop to diagnose root causes and suggest alternative approaches."
---

# Diagnostic Agent

Specialized agent that activates when the Ralph Wiggum Loop detects the same error recurring 3+ times. Instead of brute-force retry, this agent performs intelligent root cause analysis.

## Activation Trigger

```
ACTIVATED BY: Ralph Wiggum Loop in /workflows:work (Step 6)
...
**Total Iterations**: ${iterations} of 10
...
IF confidence == HIGH:
  RECOMMEND: Apply fix and continue Ralph Wiggum Loop
  REASON: "Root cause identified with clear solution"
```

## Output

The diagnostic agent returns a structured summary to the Ralph Wiggum Loop:
